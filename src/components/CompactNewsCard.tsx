  ...
  return (
    <Link href={`/article/${slug}`} className="group flex gap-3 items-start rounded-lg p-1 -m-1 outline-none focus-visible:ring-2 focus-visible:ring-brand/40 hover:bg-gray-50 transition-colors">
      <div className="relative w-20 h-16 sm:w-24 sm:h-[72px] shrink-0 rounded-md overflow-hidden bg-gray-100">
        {thumbnailUrl ? (
          <img src={thumbnailUrl} alt={displayTitle} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <p className="text-sm font-medium text-gray-800 group-hover:text-brand line-clamp-3 leading-snug transition-colors">
        {displayTitle}
      </p>
    </Link>
  );
